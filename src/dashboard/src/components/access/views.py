...
from main.models import SIPArrangeAccessMapping, ArchivesSpaceDOComponent, ArchivesSpaceDOComponentPairing
...
def digital_object_components(client, request, system='archivesspace', record_id=''):
    ...
    if request.method == 'POST':
        ...
        components = list(ArchivesSpaceDOComponent.objects.filter(resourceid=record_id).values('id', 'label', 'title'))
        ...
        try:
            ...
            response = {
                'success': False,
                'message': 'No digital object component exists with the specified ID: {}'.format(record_id),
            }
            return helpers.json_response(response, status_code=404)
        else:
...
def _fetch_paths_from_request(request):
    ...
    if not isinstance(paths, list):
        raise ValueError('Paths parameter must be an array.')
    if len(paths) == 0:
...
def digital_object_components_files(client, request, system='archivesspace', record_id='', component_id=''):
    ...
    if request.method == 'POST':
        ...
        paths = [p[0] for p in
                 ArchivesSpaceDOComponentPairing.objects.filter(component_id=component_id).values_list('relative_path')]
